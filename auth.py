...
def register():
    if request.method == "POST":
        ...
        if question and not answer:
            flash("Please provide an answer to the security question.")
        ...
        answer_hash = generate_password_hash(answer.lower()) if answer else None

        db.execute(
            """INSERT INTO users (email, password_hash, role, security_question, security_answer_hash)
               VALUES (?, ?, ?, ?, ?)""",
            (email, generate_password_hash(password), role, question or None, answer_hash),
        )
        db.commit()
...
def security_check():
    if request.method == "POST":
        ...
        return redirect(url_for("auth.answer_security_question", user_id=user["id"]))
...
def answer_security_question(user_id):
    db = get_db()
    ...
    if request.method == "POST":
        ...
        if stored and check_password_hash(stored["security_answer_hash"], answer):
            session["reset_user_id"] = user_id
            return redirect(url_for("auth.reset_password_security"))

        flash("Incorrect answer.")
        return redirect(url_for("auth.answer_security_question", user_id=user_id))
